# Posting Queue Schema — Medical Spa Social Media Automation

This document defines the structure, constraints, and processing rules for the Google Sheets tab named **Posting Queue**.

It serves as the authoritative data contract used by the automation workflow.

---

## Purpose

The Posting Queue spreadsheet acts as the **system of record** for all scheduled social media posts.

Each row represents a single post and progresses through defined processing states as it moves through the workflow.
...
- Only rows marked **Pending** are eligible for processing
- Status fields are updated automatically by the workflow
- The spreadsheet maintains a complete posting history
